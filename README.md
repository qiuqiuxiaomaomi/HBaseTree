...
MapReduce的执行地点
    Hadoop的块备份策略对上层用户十分不透明，这个过程是自动化完成的，用户不用为其操行，HBase依靠这个副本策略对上层提供持久化存储。

    MapReduce文档指出任务在靠近其数据的地方执行，为了达到这一点，HDFS中的大文件被切分成一些小的块，默认大小为64MB，每块的数据被指定到一个Map任务处理，这也说明较大的块会减少所需MAP的数目，因为MAP的数据由需要处理的块的数目决定，Hadoop知道块的位置，所以会直接在其对应位置上运行Map任务（实际上由于每个块都有若干副本，副本数量默认为3，框架可以在这三个中任意选择一个执行，用于负载均衡），这是MapReduce保证数据本地计算的方法。

    HBASE将数据存储在HDFS上，存储过程对于用户来说是透明的，HBASE通过数据文件HFile和它的日志文件WAL来实现。

    HDFS可以根据数据的使用位置智能地防止数据的副本，它自带一个副本位置策略，该策略可以再写入数据时先让数据写入到辅助定位服务器，收到数据的节点先比较自己的服务器名称和写数据的程序所在的机器是不是一样，如果一样就将数据写入本地磁盘，然后数据的一个副本就被送到同一个机架上的另一台服务器，另一个送到远程的机架--假设HDFS启用了机架感知，如果没有启用，额外的副本会被放置在集群中负载量最轻的节点上。

表拆分
    当用户使用MapReduce作业从表中读取数据时，实质上使用TableInputFormat取得数据，它符合MapReduce框架，
</pre>

<pre>
架构
   B+树
       B+树能够提供高效的范围扫描功能，这得益于它的叶节点相互连接并且按照主键有序，扫描你时避免了耗时的遍历树操作，这也是B+树被关系型数据库用作索引的原因之一，

       添加一个叶节点需要更新叶节点页表并添加一个新的索引项，对于一个固定大小的页表来说，页表大小超过容量限制时就会产生问题，这时需要将该页表拆分成两个新的页表，同时更新原页表的父节点，并使其指向刚创建的两个新节点，现在的问题是两个页表在硬盘中并不一定是彼此相邻的，如果进行一次范围查询，则需要读取多个磁盘上不连续甚至可能像个很远的叶节点页表，这也是为什么大部分基于B+树的设计中都能找到一组被称为optimize table(优化表)命令的原因，B+树这种数据组织方式只是简单的按顺序把表重写，从而使表的范围查询编程了磁盘的多段连续读取。


       LSM树
           LSM树则按照另一种方式组织数据，输入数据首先被存储在日志文件，这些文件内的数据完全有序，当有日志文件被修改时，对应的更新会被先保存在内存中来加速查询。

           当系统经历过许多次数据修改，且内存空间主键被占满后，LSM树会把有序的键-记录对写入磁盘，同时创建一个新的数据存储文件。此时因为最近的额修改都被持久化了，内存中保存的最近更新就可以被丢弃了。

           存储文件的组织与B树相似，不过其为磁盘顺序读取做了优化，所有节点都是满的并按页存储，修改数据文件的操作通过滚动合并完成，也就是说，系统将现有的页与内存刷写数据混合在一起进行管理，直到数据块达到它的容量。

           多次数据刷写之后创建许多数据存储文件，后台线程就会自动将小文件聚合成大文件，这样磁盘查找就会被限制在少数几个数据存储文件中，磁盘上的树结构也可以拆分成独立的小单元，这样更新就可以被分散到多个数据存储文件中，所有的数据存储文件都按键排序，所以没有必要在存储文件中为新的键预留位置。

  B树和LSM树最主要的区别在于他们的结构如果利用硬件，特别是磁盘。

  在没有太多的修改时，B+树表现的很好，因为这些修改要求执行高代价的优化操作以保证查询能在优先时间完成，在任意位置添加数据的规模越大，速度越快，这些页称为碎片的速度就越快，最后，用户写入的速度就越快。由于更新跟删除以磁盘寻到的速率完成，这就强制用户就范于磁盘提供的较差的性能指标。

  LSM树以磁盘传输速率工作并能较好的扩展以处理大量的数据，它们使用日志文件和内存存储来将随机写转换成顺序写，因此也能保证稳定的数据插入速率。
</pre>

<pre>
Zookeeper
    HBASE使用Zookeeper作为协同服务组件，其主要功能包括跟踪region服务器，保存root region的地址等，
</pre>

<pre>
HBase复制
     HBase复制是一种在不提供HBASE部署中复制数据的方法，它可以作为一种灾难恢复的方法，并且可以提供HBASE层的高可用性，同时在实际应用中，例如，将数据从一个面向页面的集群复制到一个MapReduce集群，后者可以同时处理新数据和历史数据，然后再自动将数据传回面向页面请求的集群。

HBASE复制中最基本的架构模式是主推送，因为每个region服务器都有自己的WAL，所以很容易保存现在正在复制的位置，其类似于其他众所周知的解决方案，来自每个region服务器的HLog是复制的基础，并且只要他们需要将数据复制到集群，它们就保存在HDFS中，每个region服务器从它需要的最老的日志开始复制，同时在Zookeeper中保存在HDFS中，每个regiono服务器从它需要的最老的日志开始复制，同时在Zookeeper中保存当前回复的位置来简化错误回复，每个从集群回复的位置可能不同，但它们处理的HLog队列内容是相同的。
</pre>